...
BOOST_AUTO_TEST_CASE(testPadeAgainstAdamsRiccati) {
    ...
    // the empirically relevant regime
    for (const Real hurst : {0.05, 0.1, 0.2}) {
        ...
        for (const Real u : {0.5, 1.0, 2.0, 5.0, 10.0, 20.0}) {
            for (const Time t : {0.25, 1.0, 2.0}) {
                ...
                const std::complex<Real> ref{adams->riccatiSolution(z, t)};
                const std::complex<Real> approx{pade->riccatiSolution(z, t)};
        ...
        for (const auto& optionType : {Option::Call, Option::Put}) {
            for (const Real strike : {80.0, 90.0, 100.0, 110.0, 120.0}) {
                for (const Time t : {0.25, 1.0, 2.0}) {
                    ...
                    // 1 to 30; the rational approximation is designed for
                    // exactly this calibration domain
                    const Real tol{2e-2};
...
BOOST_AUTO_TEST_CASE(testPadeStructuralProperties) {
    BOOST_TEST_MESSAGE(
        "Testing put-call parity and no-arbitrage bounds of the Pade "
        "engine...");
    ...
    // put-call parity is exact by construction of the pricing formula
    for (const Real strike : {70.0, 100.0, 130.0}) {
        for (const Time t : {0.25, 1.0, 2.0}) {
            const Real call{engine->priceVanillaPayoff(
                ext::make_shared<PlainVanillaPayoff>(Option::Call, strike), t)};
            const Real put{engine->priceVanillaPayoff(
                ext::make_shared<PlainVanillaPayoff>(Option::Put, strike), t)};
            ...
            const Real tol{1e-6};
            if (std::fabs((call - put) - expected) > tol)
                BOOST_ERROR("Pade engine violates put-call parity"
                            << "\n    strike:     " << strike
                            << "\n    maturity:   " << t
                            << "\n    call - put: " << call - put
                            << "\n    expected:   " << expected);
